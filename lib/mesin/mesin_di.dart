...
class mesin_di extends StatefulWidget {
  const mesin_di({Key? key}) : super(key: key);

  @override
  _mesin_di createState() => _mesin_di();
}

class _mesin_di extends State<mesin_di> {
  List<Mesin>data = [];
  ...
  Future generatedata() async {
    try{
      final response = await http.get(Uri.parse(sheet_api.URL_mesin_di));
