...
class mesin_len extends StatefulWidget {
  const mesin_len({Key? key}) : super(key: key);

  @override
  _mesin_len createState() => _mesin_len();
}

class _mesin_len extends State<mesin_len> {
  List<Mesin>data = [];
  ...
  Future generatedata() async {
    try{
      final response = await http.get(Uri.parse(sheet_api.URL_mesin_len));
  ...
  Widget build(BuildContext context){
    return Scaffold(
      appBar: AppBar(
        title: const Text('Kapabilitas Mesin Len'),
