...
class mesin_pal extends StatefulWidget {
  const mesin_pal({Key? key}) : super(key: key);

  @override
  _mesin_pal createState() => _mesin_pal();
}

class _mesin_pal extends State<mesin_pal> {
  List<Mesin>data = [];
  ...
  Future generatedata() async {
    try{
      final response = await http.get(Uri.parse(sheet_api.URL_mesin_pal));
