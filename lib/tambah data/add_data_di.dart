...
class AddData_di extends StatefulWidget {
  const AddData_di({Key? key}) : super(key: key);

  @override
  _AddData_di createState() => _AddData_di();
}

enum pilihan_upload { uploadFile, googleDriveLink }
class _AddData_di extends State<AddData_di> {
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      List<String> kategori_mesin = selectedItems_kategori_mesin.map((
          item) => '"$item"').toList();
      List<String> raw_material = selectedItems_raw_material.map((
          item) => '"$item"').toList();
      String? data_sheet;
      if (linkdrive == true) {
      ...
      http.Response response = await sheet_api.tambahdata(
          nama_table,
          id,
          nama_aset,
          jenis_aset,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        title: const Text("Tambah data DI"),
  ...
  Future<http.Response> addgambar(String imagePath, String id, String userEdit) async {
    final url = Uri.parse(sheet_api.URL_addgambar_di);
