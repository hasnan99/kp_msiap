...
import 'package:kp_msiap/chart/bar_chart_dahana.dart';
import 'package:kp_msiap/chart/pie_chart_dahana.dart';
...
class Table_dahana extends StatefulWidget {
  const Table_dahana({Key? key}) : super(key: key);

  @override
  _Table_dahana createState() => _Table_dahana();
}

class _Table_dahana extends State<Table_dahana> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Asset Table Dahana'),
        actions: [
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                bool cleardata= await Navigator.push(context,
                    MaterialPageRoute(builder: (context)=>barChartdahana(data_chart: cellValues,)));
                if(cleardata==true){
                  cellValues.clear();
                }
              },
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                bool cleardata= await Navigator.push(context,
                    MaterialPageRoute(builder: (context)=>pieChartdahana(data_chart: cellValues,)));
                if(cleardata==true){
                  cellValues.clear();
                }
              },
...
class _dataexcel extends DataGridSource {
  ...
  Widget? buildTableSummaryCellWidget(
    ...
    else if(summaryColumn?.summaryType==GridSummaryType.sum && summaryColumn?.columnName != 'No'){
    ...
    else if(summaryColumn?.summaryType==GridSummaryType.average && summaryColumn?.columnName != 'No'){
      final formatter = NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ');
      return Container(
        padding: const EdgeInsets.all(5.0),
        alignment: Alignment.center,
